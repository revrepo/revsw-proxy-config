...
describe('Proxy freshly domain control', function () {
    ...
    it('should check HTTP GET request headers', function (done) {
        tools.getHostRequest(testHTTPUrl, '/cache/5', newDomainName).then(function (res, rej) {
            ...
            if (res.header['cache-control']) { res.header['cache-control'].should.equal('public, max-age=5'); }
            res.header.should.have.property(['x-rev-cache']);
            if (res.header['x-rev-cache']) { res.header['x-rev-cache'].should.equal('MISS'); }
            res.header.should.have.property(['x-rev-beresp-ttl']);
            if (res.header['x-rev-beresp-ttl']) { res.header['x-rev-beresp-ttl'].should.equal('5.000'); }
            // quic test
    ...
    it('should check HTTPS GET request headers', function (done) {
        tools.getHostRequest(testHTTPSUrl, '/cache/60', newDomainName).then(function (res, rej) {
            ...
            //console.log(res.header);
            res.header.should.have.property(['cache-control']);
            if (res.header['cache-control']) { res.header['cache-control'].should.equal('public, max-age=60'); }
            res.header.should.have.property(['x-rev-cache']);
            if (res.header['x-rev-cache']) { res.header['x-rev-cache'].should.equal('MISS'); }
            res.header.should.have.property(['x-rev-beresp-ttl']);
            if (res.header['x-rev-beresp-ttl']) { res.header['x-rev-beresp-ttl'].should.equal('60.000'); }
            // quic test
            res.header.should.not.have.properties(['alternate-protocol', 'alt-svc']);
            // rum test
            res.text.should.not.match(rumBeaconString);
    ...
    it('should check HIT cache header and sleep 65 second', function (done) {
        tools.getHostRequest(testHTTPUrl, '/cache/5', newDomainName).then(function (res, rej) {
            ...
            //console.log(res.header);
            res.header.should.have.property(['x-rev-cache']);
            if (res.header['x-rev-cache']) { res.header['x-rev-cache'].should.equal('HIT'); }
            setTimeout(function(){
            done();
            }, 65000);
        });
    ...
    it('should check set header requests', function (done) {
        tools.getHostRequest(testHTTPUrl, '/response-headers?Set-Cookie=k1%3Dv1', newDomainName).then(function (res, rej) {
            if (rej) {
                throw rej;
            }
            //console.log(res.header);
            ...
            if (res.header['x-rev-cache']) { res.header['x-rev-cache'].should.equal('MISS'); }
            res.header.should.have.property(['x-rev-beresp-ttl']);
            if (res.header['x-rev-beresp-ttl']) { res.header['x-rev-beresp-ttl'].should.equal('0.000'); }
            done();
        });
    });
