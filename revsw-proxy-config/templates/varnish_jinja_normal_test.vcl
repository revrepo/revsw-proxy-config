...
# BEGIN SITE 'test_server_2'
backend behttp_test___server___2 {
    .host = "127.0.0.1";
    .port = "9080";
    .probe = {
        .request =
...
backend behttps_test___server___2 {
    .host = "127.0.0.1";
    .port = "9443";
    .probe = {
        .request =
...
# END SITE 'test_server_1'
# BEGIN SITE 'test_server_2'
backend becustom_test___server___2_test_bakend13123 {
	.host = "test-backends-url.com13123";
	.port = "80";
	.preresolve_dns = 0;
	  dircustom_test___server___2_test_bakend13123.backend()
}
# END SITE 'test_server_2'
...
sub vcl_init {
    ...
    # END SITE 'test_server_1'
    # BEGIN SITE 'test_server_2'
    new dircustom_test___server___2_test_bakend13123 = directors.rev_dns();
    if (dircustom_test___server___2_test_bakend13123.set_backend(becustom_test___server___2_test_bakend13123)) {}
    # END SITE 'test_server_2'
    # End custom VCL backends
}
...
sub vcl_hash {
    ...
    # END SITE 'test_server_1'
    # BEGIN SITE 'test_server_2'
    if (req.http.host == "test_server_2") {
            test-hash
    }
    # END SITE 'test_server_2'
...
# END SITE 'test_server_1'
# BEGIN SITE 'test_server_2'
acl purgehttp_test_server_2 {
    # Purge requests are only allowed from localhost.
    "localhost";
    "127.0.0.1";
      "http://test-optimizer-http-url.com";
}
acl purgehttps_test_server_2 {
    # Purge requests are only allowed from localhost.
    "localhost";
    "127.0.0.1";
      "https://test-optimizer-https-url.com";
...
sub vcl_recv {
    ...
    chromelogger.log("recv " + req.xid + ": delrsp_cook=" + revvar.get_bool(true, 2));
    chromelogger.log("recv " + req.xid + ": ignall_cook=" + revvar.get_bool(true, 5));
    chromelogger.log("recv " + req.xid + ": ignoreq_cook=" + revvar.get_string(true, 6));
    chromelogger.log("recv " + req.xid + ": keepreq_cook=" + revvar.get_string(true, 7));
    chromelogger.log("recv " + req.xid + ": rgxdelrsp_cook=" + revvar.get_string(true, 1));
    ...
    if (std.port(server.ip) == 8080) {
        set req.backend_hint = behttp_test___server___2;
    }
    else {
        set req.backend_hint = behttps_test___server___2;
    }
    ...
    # Block 3d: Verify the ACL for an incoming purge request and handle it.
    if (req.method == "PURGE") {
        if ((std.port(server.ip) == 8080 && client.ip !~ purgehttp_test_server_2) ||
            (std.port(server.ip) == 8443 && client.ip !~ purgehttps_test_server_2)) {
            return (synth(405, "Not allowed."));
        }
        return (purge);
    }
...
sub vcl_backend_fetch {
    ...
    # END SITE 'test_server_1'
    # BEGIN SITE 'test_server_2'
    if (bereq.http.host == "test_server_2") {
            test-backend_fetch
    }
    # END SITE 'test_server_2'
...
sub vcl_backend_response {
    ...
    if (revvar.get_string(false, 3)) {
        chromelogger.log("backend_response " + bereq.xid + ": saved " + revvar.get_string(false, 3));
    }
    ...
    if (bereq.http.X-Orig-Host == "test-url.com" || bereq.http.X-Orig-Host ~ "^12313$") {
        ...
        if (bereq.url ~ "^test$") {
          ...
          revvar.set_duration(false, 8, 1s);
    ...
    chromelogger.log("backend_response " + bereq.xid + ": del_age=" + revvar.get_bool(false, 0));
    chromelogger.log("backend_response " + bereq.xid + ": delay_age=" + revvar.get_bool(false, 15));
    ...
    chromelogger.log("backend_response " + bereq.xid + ": delreq_cook=" + revvar.get_bool(false, 1));
    chromelogger.log("backend_response " + bereq.xid + ": delrsp_cook=" + revvar.get_bool(false, 2));
    chromelogger.log("backend_response " + bereq.xid + ": ignall_cook=" + revvar.get_bool(false, 5));
    chromelogger.log("backend_response " + bereq.xid + ": ignoreq_cook=" + revvar.get_string(false, 6));
    chromelogger.log("backend_response " + bereq.xid + ": keepreq_cook=" + revvar.get_string(false, 7));
    chromelogger.log("backend_response " + bereq.xid + ": rgxdelrsp_cook=" + revvar.get_string(false, 1));
    chromelogger.log("backend_response " + bereq.xid + ": grace=" + revvar.get_duration(false, 18));
